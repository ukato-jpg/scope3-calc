...
function createEmpty(): Cat8Entry {
  return {
...
export function Cat8Page() {
  const entries = useCategoryStore((s) => s.data.cat8)
  ...
  function setEntries(next: Cat8Entry[]) { update('cat8', next) }
  function addRow() { setEntries([...entries, createEmpty()]) }
  function removeRow(id: string) { setEntries(entries.filter((e) => e.id !== id)) }

  function updateRow(id: string, field: keyof Cat8Entry, value: string | number | null) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">カテゴリ8. リース資産（上流）</h1>
        <p className="text-muted-foreground mt-1">
          自社がリースを受けている資産の使用に伴う排出量
        </p>
      </div>
