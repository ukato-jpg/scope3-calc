...
import { calculateCat12 } from '@/engine/cat12'
import { sumEmissions } from '@/engine/common'
import type { Cat12Entry } from '@/types/categories'
import type { IdeaRecord } from '@/types/idea'

function createEmpty(): Cat12Entry {
  return {
    id: newId(), name: '', wasteAmount: 0,
    treatmentMethod: '', ideaProductCode: '',
  }
}
...
export function Cat12Page() {
  const entries = useCategoryStore((s) => s.data.cat12)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">カテゴリ12. 販売した製品の廃棄</h1>
        <p className="text-muted-foreground mt-1">
          販売した製品が最終的に廃棄される際の処理に伴う排出量
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-48">廃棄物名</TableHead>
                <TableHead className="w-28 text-right">処理量 [t]</TableHead>
                <TableHead className="w-32">処理方法</TableHead>
            ...
            <TableFooter>
              <TableRow>
                <TableCell colSpan={5} className="font-bold">合計</TableCell>
